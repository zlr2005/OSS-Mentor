...
class ProfileServiceV05Tests(
    ...
    def setUp(self) -> None:
        ...
        self.user_id = IdentityStore(
            self.storage
        ).create_user(
            github_user_id=1001,
            github_login="fixture-dev",
        )
    ...
    def test_manual_profile_works_without_github(
        ...
        self.assertEqual(
            saved,
            self.service.profile_for_user(
                self.user_id
            ),
        )

        contract = self.service.profile_contract(
            "fixture-user"
        )
    ...
    def test_accept_explicit_skill_suggestion_updates_profile(
        ...
        resolved_target = next(
            ...
            if item["profile_field_suggestion_id"]
            == target["profile_field_suggestion_id"]
        )
        self.assertEqual(
            "accepted",
